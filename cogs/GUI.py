...
class GUI(commands.Cog):
    ...
    async def ss(self, ctx: commands.Context, regionx: int = 0, regiony: int = 0, regionwidth: int = pyautogui.size()[0], regionheight: int = pyautogui.size()[1]):
        pyautogui.screenshot(
            region=(
                ...
                regiony,
                regionwidth,
                regionheight)).save("pyauto_resources/game_image.jpg")
        f = discord.File(
            "pyauto_resources/game_image.jpg",
    ...
    async def emu(self, ctx, game: str = "Pokemon\\FireRed", framerate: float = 1.0):
        rom_path = "R:\\mGBA\\mGBA.exe \"R:\\mGBA\\_games\\{}\\{}.gba\"".format(
            game, game[game.index("\\") + 1:])  # FireRed.gba
        subprocess.Popen(rom_path)

        await asyncio.sleep(1.5)  # Give time to open
        window = [w for w in pyautogui.getAllWindows(
        ) if w.title.startswith("mGBA")][0]
        ...
        while True:
            embed = fmte(
                ...
                d="For: {} minutes".format(
                    round((time.time() - ctx.start_time) / 60, 2))
            )
            pyautogui.screenshot(
                region=(
                    212,
                    ...
                    212,
                    1038 -
                    45)).save("pyauto_resources/game_image.jpg")
            file = discord.File(
                "pyauto_resources/game_image.jpg",
                filename="Game_Image.jpg")
