...
from auxiliary.Paginator import Paginator
...
class ItemDatabase:
    ...
    async def addItem(self, user: discord.User, itemid: str, amount: int, autoremove: bool = True) -> Mapping[str, int]:
        """
        Adds an item to the user's inventory
        Returns the user's new items
        """
        result = self.collections["items"].find_one_and_update({"userid": user.id}, {"$inc": {f"items.{itemid}": amount}}, upsert = True, return_document=True)
    ...
    async def getItems(self, user: discord.User) -> Mapping[str, int]:
        ...
        if result is None:
            self.collections["items"].insert_one(document={"userid": user.id, "items": {}})
...
class InventoryView(Paginator):
    def __init__(self, ctx: commands.Context, values: List[Tuple[str, int]], sort: Literal["amount", "name"] = "name"):
        # [(sword, 2), (idk, 13)]
        addrs = {
            "name": lambda x: x[0],
            "amount": lambda x: x[1],
    ...
    def embed(self, inter: discord.Interaction):
        embed = fmte(
            self.ctx,
            t = f"Inventory: Page `{self.position}` / `{self.maxpos or 1}`"
        )
        return embed
