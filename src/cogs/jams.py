...
from ..utils.errors import AlreadyPlaying, AlreadyConnected, MissingArguments, NotConnected
...
class Jams(BaseCog):
    ...
    async def _create_node(self, ctx: BuilderContext):
        node = await wavelink.NodePool.create_node(
            ...
            identifier=f"{ctx.guild.id}::{int(time.time())}"
        )
        embed = await ctx.format(
            title="Node Created",
            desc=f"**Identifier:** {node.identifier}\n"
        )
    ...
    async def connect(self, ctx: BuilderContext, queue_type: QueueType):  
        ...
        try:
            await channel.connect(cls=BuilderWave(ctx.author, QueueType(queue_type)), self_deaf=True)
        except ClientException as err:
            raise AlreadyConnected("I am already connected to a voice channel! Please ask the the owner to disconnect me")
    ...
    async def playing(self, ctx: BuilderContext):
        ...
        if not player.is_playing():
            if player.queue.is_empty:
                embed = await ctx.format(
                    ...
                    desc="Use `/jams queue add <query>` to add a song to the queue!"
                )
                await ctx.send(embed=embed)
                return
        ...
        embed.description += f"**Is Playing:** `{'Yes' if player.is_playing() else 'No'}`\n"
    ...
    async def view(self, ctx: BuilderContext):
        ...
        embed = await ctx.format(
            title=f"Viewing `{len(player.queue)}` Tracks",
            desc=""
        )
    ...
    async def add(self, ctx: BuilderContext, track: wavelink.YouTubeTrack, location: Optional[app_commands.Choice[int]] = 1):
