...
class CommandView(discord.ui.View):
    def __init__(self, bot: commands.Bot, ephemeral: bool, cog: commands.Cog, pagesize: int, *, timeout: Optional[float] = 180):
        self.bot = bot
        self.ephemeral = ephemeral

        self.vals = sorted(MixedHelp(bot).explode(cog.get_commands()), key = lambda c: c.qualified_name)
        self.pos = 0
    ...
    async def back(self, inter: discord.Interaction, _: Any):
        ...
        if self.pos <= 0:
            self.pos = self.maxpos - 1  # Loop to end
    ...
    async def next(self, inter: discord.Interaction, _: Any):
        ...
        if self.pos >= self.maxpos:
            self.pos = 0  # Loop back to beginning
    ...
    def embed(self, inter: discord.Interaction):
        return fmte_i(
            inter,
            t="Commands: Page `{}` of `{}`".format(self.pos + 1, self.maxpos)
        )
    
    def add_fields(self, embed: discord.Embed):
        for command in self.vals[self.pagesize * \
            (self.pos):self.pagesize * (self.pos + 1)]:
