...
class Table:
    def __init__(self, database: str, table: str, values: Mapping[str, Type], *, if_not_exists: bool = True) -> None:
        ...
        values: str = ", ".join(["{} {}".format(k, v) for k, v in values.items()])
    ...
    def select(self, *, values: List[str] = "*", conditions: List[str] = {}) -> sqlite3.Cursor:
        """Gets all `values` from all rows where all `conditions` are True. Retuns a `sqlite3.Cursor` for easier fetching."""
        values: str = ", ".join(values) if type(values) == List else "*"
    ...
    def update(self, values: List[str], *, conditions: List[str] = {}) -> Self:
        ...
        values: str = ", ".join(["{}={}".format(list(self.values.keys())[c], v) for c, v in enumerate(values)])
        where = "WHERE " if len(conditions) > 0 else ""
        conditions: str = ", ".join(conditions)
